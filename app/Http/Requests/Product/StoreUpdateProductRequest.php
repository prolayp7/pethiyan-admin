...
class StoreUpdateProductRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'tax_group_id' => 'nullable|exists:tax_classes,id',
            ...
            'total_allowed_quantity' => 'nullable|integer|min:0',
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            $this->validateMainImageFile($validator);
            $this->validateAdditionalImageFiles($validator);
    ...
    /**
     * Validate main image only when a new file is uploaded.
     * Edit-mode preloaded values should not trigger image validation.
     */
    protected function validateMainImageFile($validator): void
    {
        ...
        if ($fileValidator->fails()) {
            foreach ($fileValidator->errors()->all() as $message) {
                $validator->errors()->add('main_image', $message);
            }
        }
    }

    /**
     * Validate only real uploaded files for additional images.
     * Preloaded/edit-mode string entries should be ignored.
     */
    protected function validateAdditionalImageFiles($validator): void
    {
        if (!$this->hasFile('additional_images')) {
            return;
        }

        $files = $this->file('additional_images', []);
        if (!is_array($files)) {
            $files = [$files];
        }
        ...
        foreach ($files as $index => $file) {
            ...
            if ($fileValidator->fails()) {
                foreach ($fileValidator->errors()->all() as $message) {
                    $validator->errors()->add("additional_images.$index", $message);
                }
            }
        }
    }
...
//        }
    ...
    protected function validatePricingFields($validator, array $row, string $prefix): void
    {
        $fieldValidations = [
            ...
            'cost' => [
                'condition' => isset($row['cost']) && $row['cost'] !== '' &&
                    (!is_numeric($row['cost']) || $row['cost'] < 0),
                'message' => __('labels.cost_required_numeric'),
            ],
            'stock' => [
                'condition' => isset($row['stock']) && $row['stock'] !== '' && (!is_numeric($row['stock']) || intval($row['stock']) < 0),
