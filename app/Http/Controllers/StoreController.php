...
class StoreController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if ($this->getPanel() === 'admin') {
            ...
            $this->editPermission = $this->hasPermission(AdminPermissionEnum::STORE_EDIT());
            $this->deletePermission = $this->hasPermission(AdminPermissionEnum::STORE_DELETE());
            $this->createPermission = $this->hasPermission(AdminPermissionEnum::STORE_CREATE());
        }
    }
    ...
    public function store(StoreStoreRequest $request): JsonResponse
    {
        try {
            ...
            $request->validated();
            $user = auth()->user();
            // Admin panel: always use Pethiyan seller (ID=1)
            $seller = $this->getPanel() === 'admin'
                ? Seller::find(1)
                : $user->seller();
            if (!$seller) {
                return ApiResponseType::sendJsonResponse(success: false, message: 'Seller not found');
            ...
            if ($this->getPanel() === 'admin') {
                $responseData['redirect_url'] = route('admin.sellers.store.index');
            }
            return ApiResponseType::sendJsonResponse(success: true, message: 'Store created successfully', data: $responseData);
    ...
    public function update(UpdateStoreRequest $request, $id): JsonResponse
    {
        try {
            ...
            return ApiResponseType::sendJsonResponse(true, 'Store updated successfully', $responseData);
        } catch (AuthorizationException $e) {
            DB::rollBack();
    ...
    public function StoreList(Request $request): JsonResponse
    {
        ...
        // Admin panel: filter by seller_id query param if provided, otherwise return all stores
        if (!$seller) {
        ...
        $stores = $stores->map(function ($store) use ($stateMap) {
