...
window.DatatableUtils = {
    ...
    refreshDatatable: function (tableId = null) {
        if (typeof jq !== 'function' || !jq.fn?.DataTable) {
            return;
        }

        if (tableId && jq.fn.DataTable.isDataTable('#' + tableId)) {
            // Refresh specific table if ID is provided and table exists
            jq('#' + tableId).DataTable().ajax.reload(function () {
            ...
            const datatables = jq.fn.dataTable.tables();
            if (datatables.length > 0) {
                jq(datatables).each(function () {
                    jq(this).DataTable().ajax.reload(function () {
...
if (typeof jq === 'function' && jq.fn?.DataTable) {
    jq(document).ready(function () {
        ...
        jq(document).on('ajaxStart', function () {
            jq('#datatable-loading').removeClass('d-none');
        }).on('ajaxStop', function () {
            jq('#datatable-loading').addClass('d-none');
        });
        // Reset filters
        jq('#resetFilters').on('click', function () {
            jq('#statusFilter, #brandFilter, #categoryFilter').val('');
            jq('#reportrange').val('');
        ...
        datatableElements.forEach((element) => {
            ...
            let defaultOptions = {
                ...
                initComplete: function () {
                    let searchInput = jq('.dt-search input');
                    searchInput.removeClass('form-control-sm').addClass('ms-0').attr('placeholder', 'Search...');
                    jq('.dt-search label').contents().filter(function () {
            ...
            if (Array.isArray(customOptions)) {
                customOptions.forEach(option => {
                    defaultOptions = jq.extend(true, {}, defaultOptions, option);
                });
            } else {
                defaultOptions = jq.extend(true, {}, defaultOptions, customOptions);
            }
            jq(element).DataTable(defaultOptions);
        });
    });
    jq(document).ready(function () {
        // Add click event listener for refresh button
        jq('#refresh, .refresh-table').on('click', function () {
            // Try to find the closest datatable to the clicked button
            const closestTable = jq(this).closest('.card').find('table.dataTable');
