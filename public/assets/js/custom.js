...
function handleDelete(event, selector, urlPrefix, confirmationText) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const explicitUrl = deleteBtn.getAttribute('data-url');
            const url = explicitUrl || `${base_url}${urlPrefix}${id}`;
            fetch(url, {
                method: 'DELETE',
                headers: {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (selectTaxGoup) {
        window.TomSelect && new TomSelect(document.getElementById('select-tax-group'), {
            copyClassesToDropdown: false,
            dropdownParent: "body",
