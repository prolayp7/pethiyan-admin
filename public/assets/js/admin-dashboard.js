document.addEventListener("DOMContentLoaded", function () {
    ...
    if (window.ApexCharts && document.getElementById("chart-repeated-customers")) {
        const repeatedCustomersData = dashboardData.repeatedCustomersData;
        const dates = repeatedCustomersData.daily.map(item => item.date);
        const counts = repeatedCustomersData.daily.map(item => item.count);

        new ApexCharts(document.getElementById("chart-repeated-customers"), {
            chart: {
                type: "line",
                fontFamily: "inherit",
                height: 60,
            ...
            labels: dates,
            colors: ["color-mix(in srgb, transparent, var(--tblr-green) 100%)"],
            legend: {
                show: false,
            },
        }).render();
    ...
    document.querySelectorAll('.dropdown-menu a[data-period]').forEach(item => {
        item.addEventListener('click', function (e) {
            ...
            if (toggle.classList.contains('new-users-period')) {
                type = 'new_users';
            } else if (toggle.classList.contains('repeated-customers-period')) {
                type = 'repeated_customers';
...
function fetchDashboardData(type, days) {
    axios.get(`/admin/dashboard/data?type=${type}&days=${days}`)
        .then(function (response) {
            ...
            } else if (type === 'repeated_customers') {
                updateRepeatedCustomersData(data);
            } else if (type === 'sales') {
...
function updateRepeatedCustomersData(data) {
    const repeatedCustomersCountElement = document.getElementById('repeated-customers-count');
    if (repeatedCustomersCountElement) {
        repeatedCustomersCountElement.textContent = data.count;
    }

    const trendElement = document.getElementById('repeated-customers-trend');
    if (trendElement) {
    ...
    if (window.ApexCharts) {
        const chartElement = document.getElementById('chart-repeated-customers');
        if (chartElement) {
