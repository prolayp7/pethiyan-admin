...
class TaxClassController extends Controller
{
    ...
    public function getTaxClasses(Request $request): JsonResponse
    {
        ...
        $columns = ['id', 'title', 'rates', 'is_active', 'created_at'];
        ...
        $data = $query
            ...
            ->map(function ($taxClass) use ($editPermission, $deletePermission) {
                $slabColors = ['0' => 'secondary','5' => 'success','12' => 'info','18' => 'warning','28' => 'danger'];
                $rateBadges = $taxClass->taxRates->map(function ($rate) use ($slabColors) {
                    $slab  = $rate->gst_slab ?? null;
                    $color = $slabColors[$slab] ?? 'secondary';
                    return "<span class=\"badge bg-{$color}-lt me-1\">{$rate->title}</span>";
                })->implode('');

                $statusBadge = ($taxClass->is_active ?? true)
