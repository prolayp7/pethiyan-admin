<div class="d-flex justify-content-start align-items-center">
    ...
    @if(($duplicatePermission ?? false) && !empty($duplicateRoute))
        <form action="{{ $duplicateRoute }}" method="POST" class="d-inline-block me-2"
              onsubmit="return confirm('Create a copy of {{ addslashes($title) }}?');">
            @csrf
            <button type="submit" class="btn btn-outline-secondary p-1" title="Copy {{ $title }}">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                     stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                     class="icon icon-tabler icons-tabler-outline icon-tabler-copy m-0">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                    <path d="M7 7m0 2a2 2 0 0 1 2 -2h9a2 2 0 0 1 2 2v9a2 2 0 0 1 -2 2h-9a2 2 0 0 1 -2 -2z"/>
                    <path d="M5 17h-1a2 2 0 0 1 -2 -2v-10a2 2 0 0 1 2 -2h10a2 2 0 0 1 2 2v1"/>
                </svg>
            </button>
        </form>
    @endif
