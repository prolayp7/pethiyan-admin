...
class OrderService
{
    ...
    private function finalizeOrderCreation(Order $order, Cart $cart, $user, array $data): array
    {
        ...
        // Aggregate GST totals from all order items and save on the order
        $order->refresh();
        $gstTotals = $this->gstService->orderTotals(
        ...
        $order->update([
            'total_taxable_amount' => $gstTotals['total_taxable_amount'],
            'total_cgst'           => $gstTotals['total_cgst'],
            'total_sgst'           => $gstTotals['total_sgst'],
            'total_igst'           => $gstTotals['total_igst'],
            'total_gst'            => $gstTotals['total_gst'],
        ]);
    ...
    function createOrderFromCart(User $user, Cart $cart, array $data): mixed
    {
        ...
        $order = Order::create([
            ...
            // GST: customer delivery state + supply type
            'customer_state'      => $data['address']['state'] ?? null,
            'customer_state_code' => $this->resolveStateCode($data['address']['state'] ?? null),
            'supply_type'         => $this->resolveSupplyType($cart, $data['address']['state'] ?? null),
        ]);
    ...
    function createOrderItem(Order $order, $cartItem, $storeVariant, $subtotal, $adminCommissionAmount, $promoDiscount, $taxPercent, array $gstData = [], int $stockShortage = 0, int $availableStock = 0): OrderItem
    {
        return OrderItem::create([
            ...
            // GST fields
            'hsn_code'        => $gstData['hsn_code']        ?? null,
            'gst_rate'        => $gstData['gst_rate']        ?? 0,
            'gst_type'        => $gstData['gst_type']        ?? 'intra',
            ...
            'total_tax_amount'=> $gstData['total_tax_amount']?? 0,
        ]);
    }
    ...
    private function resolveStateCode(?string $stateName): ?string
    {
        ...
        foreach (GstService::STATE_CODES as $code => $name) {
            if (mb_strtolower($name) === $needle) {
                return $code;
            }
        }

        // Try 2-letter code passed directly (e.g. "MH")
        $upper = strtoupper(trim($stateName));
        if (isset(GstService::STATE_CODES[$upper])) {
            return $upper;
        }
    ...
    /**
     * Determine intra/inter supply by comparing the customer's delivery state
     * with the state of the first store in the cart.
     */
    private function resolveSupplyType($cart, ?string $customerState): string
    {
        $customerCode = $this->resolveStateCode($customerState);
        if (empty($customerCode)) {
            return 'intra'; // safe default
        }

        $firstStore = $cart->items->first()?->store;
        if (!$firstStore || empty($firstStore->state_code)) {
            return 'intra';
        }

        return $this->gstService->supplyType($firstStore->state_code, $customerCode);
    }
